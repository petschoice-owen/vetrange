...
if ( ! function_exists( 'custom_is_woocommerce_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function custom_is_woocommerce_activated() {
 ...
 * Accounts for the custom Designer and custom Powerpack content background option.
 *
 * @since  1.6.0
 * @return string the background color
 */
function custom_get_content_background_color() {
	if ( class_exists( 'custom_Designer' ) ) {
		$content_bg_color = get_theme_mod( 'sd_content_background_color' );
		$content_frame    = get_theme_mod( 'sd_fixed_width' );
	}

	if ( class_exists( 'custom_Powerpack' ) ) {
	...
	if ( class_exists( 'custom_Powerpack' ) || class_exists( 'custom_Designer' ) ) {
...
/**
 * Apply inline style to the custom header.
 *
...
function custom_header_styles() {
	...
	$styles = apply_filters( 'custom_header_styles', $styles );
...
/**
 * Apply inline style to the custom homepage content.
 *
 * @uses  get_the_post_thumbnail_url()
 * @since  2.2.0
 */
function custom_homepage_content_styles() {
	...
	$styles = apply_filters( 'custom_homepage_content_styles', $styles );
 ...
 * @since  1.0.0
 */
function custom_adjust_color_brightness( $hex, $steps, $opacity = 1 ) {
 ...
 * @since  1.3.0
 */
function custom_sanitize_choices( $input, $setting ) {
